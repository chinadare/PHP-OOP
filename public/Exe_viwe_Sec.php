	...
	<div class="ts-main-content">
		<nav class="ts-sidebar">
			<ul class="ts-sidebar-menu">
				...
				<li class="open"><a href="Exe_viwe_Sec.php"><i class="fa fa-dashboard"></i> Activity Log</a></li>
		...
		<div class="container">
						...
						<p style="font-size:120%;">  Welcome <a href="#" ><?php echo escape($user->data()->Emp_name);  ?> [Manager] : Activity Log <a/> </p>
						<?php
						if(Input::exists()){
							if(Input::get('btndel')){

								$database = DB:: getinstance()->query("DELETE  FROM login_sec ");
					...
					<div class="panel-heading">Viwe Customers</div>
						<div class="panel-body">
							<table id="zctb" class="display table  table-bordered table-hover" cellspacing="0" width="100%">
								...
								<th>Page Name</th>
								<th>Activity</th>
								<th><form name='frmLogin' method='POST' action='#'>
										<input class="btn btn-error" type="submit" name="btndel" value="Empty Activity Log" />
									</form>
								 </th>
									...
									<?php
									$database = DB:: getinstance()->query('SELECT COP_Pid FROM login_sec ');
										...
										foreach($database->results() as $database){
										if($database->COP_Pid == "0"){
											$database = DB:: getinstance()->query('SELECT DISTINCT  login_sec.Page,login_sec.Activity,login_sec.Login_time,users.Emp_name FROM login_sec,users WHERE login_sec.Log_id=users.id');
											...
											echo	" <td> $database->Login_time </td> ";
											echo	" <td> $database->Page </td> ";
											echo	" <td> $database->Activity </td> ";
										...
										$database = DB:: getinstance()->query('SELECT DISTINCT  login_sec.Page,login_sec.Activity,login_sec.Login_time,tbl_project.pj_name,users.Emp_name FROM login_sec,users,tbl_project WHERE login_sec.COP_Pid=tbl_project.id AND login_sec.Log_id=users.id');
										...
										echo	" <td> $database->Login_time </td> ";
										echo	" <td> $database->Page </td> ";
										echo	" <td> $database->pj_name  :  $database->Activity </td> ";

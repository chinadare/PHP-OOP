...
if ($user->hasPermission('executive')) {
	...
	if(!$_SESSION['banklanttal'] ==""){	

	$banktotal = $_SESSION['banklanttal'];	
				...
				if($validation->passed()){
					...
					try {
						...
						if($jb == "1"){
						    ...
						    $database = DB:: getinstance()->query("UPDATE tbl_bank_loan SET  discount='$discount',Total_Amount_with_discount='$Total_Amount_with_discount',Deposite='$Deposite',Total_Amount_with_depo='$Total_Amount_with_depo',down_payment='$down_payment',total_payment='$total_payment',payment_starting_date='$payDate',due_date='$due_date' WHERE cus_id='$customer' ");

						     $database = DB:: getinstance()->query("UPDATE tbl_bank_loan_prograss SET  total_payment=' $total_payment' WHERE c_id='$customer' ");
						    	  ...
						    	  $user->create('login_sec',array(
							            ...
							            "Activity" =>  $discount.','.$Total_Amount_with_discount.','.$Deposite.','.$Total_Amount_with_depo.','.$down_payment.','.$payDate
						...
						Session::flash("succbank","<div class='alert alert-success'>
						    ...
						    $database = DB:: getinstance()->query("UPDATE tbl_bank_loan SET  discount='$discount',Total_Amount_with_discount='$Total_Amount_with_discount',Deposite='$Deposite',Total_Amount_with_depo='$Total_Amount_with_depo',total_payment='$total_payment',payment_starting_date='$payDate',due_date='$due_date' WHERE cus_id='$customer' ");	
						    ...
						    $database = DB:: getinstance()->query("UPDATE tbl_bank_loan_prograss SET  total_payment=' $total_payment' WHERE c_id='$customer' ");
						     ...
						     $user->create('login_sec',array(
							            ...
							            "Activity" =>  $discount.','.$Total_Amount_with_discount.','.$Deposite.','.$Total_Amount_with_depo
...
?> 	    		
	<div class="container">
		<p style="font-size:120%;">  Welcome <a href="#" ><?php echo escape($user->data()->Emp_name);  ?> [Clerk] : Customer Purchase - Bank Loan <a/> </p>
			<div class="col-md-9">
	 						...
	 						<table class="table table-bordered table-striped">
								...
								<form name="frmAddProp" method="POST" action="#">
									...
									<tr>
										<td> Total Amount</td>
										<td> 
											<input id='total' class="form-control" readonly  type="text" name="txttemtotal1" value="<?php echo $banktotal; ?>"  />
									...
									<tr>
										<td> Customer Job type </td>
										<td> 
											<Select id="jobType" class="form-control" name="cmbjobt"  onchange="if (this.value=='0'){this.form['txtpdwn'].style.visibility='hidden'}else {this.form['txtpdwn'].style.visibility='visible'}; " required>
												<option value="" selected="selected"> - Select Job Type - </option>
												<option value="0"> Geveroment </option>
									...
									<tr>
										...
										<td> 
											<input id="downPay" class="form-control"  value="0.00" type="text" name="txtpdwn" style="visibility:hidden;" />
										</td>
									</tr>
										...
										<td> 
											...
											<script>
												$(document).ready(function() {
													...
													$('#totalDeposit,#jobType').on('input change', function(){

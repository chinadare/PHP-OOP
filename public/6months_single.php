...
if ($user->hasPermission('manager')) {
	...
	if(!Input::get('txtId6mon') ==""){
	$id6 = Input::get('txtId6mon');
	$name = Input::get('txtnm6mon');
...
<body>
	...
	<div class="ts-main-content">
		...
		<p style="font-size:120%;">  Welcome <a href="#" ><?php echo escape($user->data()->Emp_name);  ?> [Manager] :  6 Months  Summery Per Month<a/> </p>
							...
							<table id="zctb" class="display table  table-bordered table-hover" cellspacing="0" width="100%">
								...
								<th>To Recover</th>
									...
									<?php
													...
													$database = DB:: getinstance()->query("SELECT *  FROM tbl_6_month_progress WHERE c_id ='$id6'"																	
																			);
																			if(!$database->count()){
																				echo "No Record";
																			}else{
